...
class TestRoundTripConsistency:
    ...
    def _resolve_ref(self, ref: str, all_symbols: list[dict]) -> list[dict]:
        ...
        if path_filter:
            candidates = [
                s for s in candidates if Path(s.get("path", "")).name == path_filter
            ]
        ...
        if len(parts) == 1:
            ...
            for sym in candidates:
                ...
                sym_container_normalized = self.server._normalize_container(
                    sym_container
                )
    ...
    def test_main_f_bug(self):
        ...
        refs = [
            self.server._generate_unambiguous_ref(s, all_symbols, "f")
        ...
        for i, ref in enumerate(refs):
            ...
            assert len(resolved) == 1, (
                f"Ref '{ref}' resolved to {len(resolved)} symbols: {resolved}"
            )
            assert resolved[0] == all_symbols[i], (
                f"Ref '{ref}' resolved to wrong symbol"
            )
    ...
    def test_same_container_name_different_meanings(self):
        ...
        refs = [
            self.server._generate_unambiguous_ref(s, all_symbols, "x")
            for s in all_symbols
        ]
        ...
        for i, ref in enumerate(refs):
            resolved = self._resolve_ref(ref, all_symbols)
            assert len(resolved) == 1, (
                f"Ref '{ref}' resolved to {len(resolved)} symbols"
            )
            assert resolved[0] == all_symbols[i]
    ...
    def test_all_refs_resolve_uniquely(self):
        ...
        for symbols in test_cases:
            refs = [
                self.server._generate_unambiguous_ref(s, symbols, s["name"])
                for s in symbols
            ]
            ...
            for i, ref in enumerate(refs):
                ...
                assert len(resolved) == 1, (
                    f"Ref '{ref}' resolved to {len(resolved)}: {resolved}"
                )
    ...
    def test_module_name_matching_is_intentional(self):
        ...
        for sym in test_symbols:
            ...
            if (
                ...
                or sym_container == container_str
            ):
                matches.append(sym)
...
class TestRefResolvesUniquely:
    ...
    def test_filename_ref_unique(self):
        ...
        assert self.server._ref_resolves_uniquely(
            "user.py:validate", symbols[0], symbols
        )
        assert self.server._ref_resolves_uniquely(
            "order.py:validate", symbols[1], symbols
        )
    ...
    def test_filename_ref_not_unique_same_file(self):
        ...
        assert not self.server._ref_resolves_uniquely(
            "utils.py:log", symbols[0], symbols
        )
    ...
    def test_line_ref_always_unique(self):
        ...
        assert self.server._ref_resolves_uniquely(
            "utils.py:10:log", symbols[0], symbols
        )
        assert self.server._ref_resolves_uniquely(
            "utils.py:50:log", symbols[1], symbols
        )
    ...
    def test_go_effective_container(self):
        ...
        assert self.server._ref_resolves_uniquely(
            "MemoryStorage.Save", symbols[0], symbols
        )
        assert self.server._ref_resolves_uniquely(
            "FileStorage.Save", symbols[1], symbols
        )
...
class TestModuleNameCollisionHandling:
    ...
    def test_module_name_match_both_directions(self):
        ...
        refs = [
            self.server._generate_unambiguous_ref(s, symbols, "helper") for s in symbols
        ]
...
class TestGoMethodDisambiguation:
    ...
    def test_go_interface_vs_implementations(self):
        ...
        refs = [
            self.server._generate_unambiguous_ref(s, symbols, "Save") for s in symbols
        ]
...
class TestResolveSymbolRoundTrip:
    ...
    def test_container_ref_resolves(self):
        ...
        # Generate refs
        ref1 = self.server._generate_unambiguous_ref(
            all_symbols[0], all_symbols, "save"
        )
        ...
        if "." in ref1 and ":" not in ref1:
            ...
            matches = [
                ...
                for s in all_symbols
                if s["name"] == name
                and self.server._get_effective_container(s) == container
            ]
