        ...
        if len(parts) == 1:
            ...
            for sym in all_symbols:
                ...
                if sym_container_normalized == container_str:
                    ...
                    logger.debug(f"  -> matched via container")
                    matches.append(sym)
                elif full_container == container_str:
                    logger.debug(f"  -> matched via full_container")
                    matches.append(sym)
                elif full_container.endswith(f".{container_str}"):
                    logger.debug(f"  -> matched via full_container.endswith")
                    matches.append(sym)
                elif container_str in sym_container_normalized:
                    logger.debug(f"  -> matched via container_str in sym_container_normalized")
                    matches.append(sym)
                elif len(container_parts) == 1 and container_parts[0] == module_name:
                    logger.debug(f"  -> matched via module_name")
                    matches.append(sym)
